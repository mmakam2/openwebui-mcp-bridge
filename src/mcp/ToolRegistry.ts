...
console.warn('[ToolRegistry] listTools failed:', r.reason?.message ?? r.reason);
}
}
...
async execute(toolCall: { function: { name: string; arguments?: string } }): Promise<{ content: string }>{
const name = toolCall.function.name;
const client = this.toolMap.get(name);
if (!client) return { content: `Tool not found: ${name}` };
let args: any = {};
